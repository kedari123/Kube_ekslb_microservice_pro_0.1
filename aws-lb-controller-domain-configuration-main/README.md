# 🚀 JDKT Batch 09 – Learn With KASTRO

> 🎯 **Master DevOps with Real-Time Projects | Become Job-Ready in 50 Days**

---

## 📌 📚 Training Overview



---

## 🛠️ ⚙️ Core DevOps Stack

<p align="center">
  🚀 Jenkins &nbsp;&nbsp;|&nbsp;&nbsp; 🐳 Docker &nbsp;&nbsp;|&nbsp;&nbsp; ☸️ Kubernetes &nbsp;&nbsp;|&nbsp;&nbsp; ⚡ GitHub Actions &nbsp;&nbsp;|&nbsp;&nbsp; 🌍 Terraform
</p>

---

## 🔧 🧰 Additional Tools Covered

<p align="center">
📦 Maven &nbsp;|&nbsp; 🐱 Tomcat &nbsp;|&nbsp; 📁 Nexus &nbsp;|&nbsp; 🔍 SonarQube &nbsp;|&nbsp; 🌐 Nginx  
📊 Prometheus &nbsp;|&nbsp; 📈 Grafana &nbsp;|&nbsp; 🔁 ArgoCD &nbsp;|&nbsp; 📚 EFK Stack  
🛡️ Istio &nbsp;|&nbsp; 🔐 Trivy
</p>

---

## 💼 🚀 Capstone Projects (Real-Time Experience)

> 🎯 Learn by building **industry-level DevOps pipelines**

### 🔹 Project Breakdown

- ⚡ **2 Projects using GitHub Actions**
  - CI/CD workflows  
  - Automated builds & deployments  

- 🔧 **2 Projects using Jenkins**
  - End-to-End pipeline automation  
  - Integration with Docker & Kubernetes  

💡 *These projects simulate real-world DevOps environments to make you industry-ready.*

---

## 🎯 🧠 What You Will Gain

✔️ Hands-on CI/CD Pipeline Implementation  
✔️ Containerization & Orchestration (Docker + Kubernetes)  
✔️ Infrastructure as Code (Terraform)  
✔️ Monitoring & Logging (Prometheus + Grafana + EFK)  
✔️ DevOps Security & Best Practices  
✔️ Real-time Project Experience  

---

## 🎓 🎥 Free Demo Sessions

- 📅 **Dates:** 07 & 08 May  
- 🕠 **Time:** 5:30 PM  

👉 *Meeting link will be shared after registration.*

---

## 📝 📢 Register Now

<p align="center">
  👉 <a href="https://forms.gle/eeijRLrDReZF2WMG7"><b>CLICK HERE TO REGISTER</b></a>
</p>

---

## 📞 📲 Contact

- 📱 **Phone:** 9700712459  

---

## ⭐ 💡 Why Learn With KASTRO?

✨ Real-time, hands-on training  
✨ Industry-focused curriculum  
✨ Project-based learning approach  
✨ Designed for **job readiness**  

---

## ⚡ Limited Seats!

> 🚨 *Enroll early to secure your spot in Batch 09!*
